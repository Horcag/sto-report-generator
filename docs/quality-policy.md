...
This is a manually requested release/acceptance signal, not a push or pull-request gate. Do not expose the self-hosted runner to untrusted code, do not add this job to GitHub-hosted CI, and do not infer Word availability from a `windows-latest` runner label. The Word step has a bounded timeout because Microsoft does not support unattended Office automation as a server-side workload.
