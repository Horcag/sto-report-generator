...
Перед сборкой отчёта выполните [редакторский проход по русскому тексту](docs/report-authoring.md#редакторский-проход-по-русскому-тексту)
в исходных Markdown-файлах. Для текста, который пишет или редактирует агент, это обязательный шаг с [встроенным навыком
`humanizer-ru`](.agents/skills/humanizer-ru/SKILL.md). Пользователь может пройти тот же список вручную. Команды
`check` и `generate` напоминают о шаге, но не переписывают текст и не подтверждают, что редактура выполнена.
Встроена [полная копия релиза v3.36.4](docs/humanizer-ru-integration.md) с исходным кодом, скриптами и тестами.
...
В формулах роли символов задаются явно: `\stovec{a}` для строчного вектора,
`\stomat{A}` для прописной матрицы, `\stotemp{K}` для единицы температуры,
`\stoelem{Fe}` для химического элемента и `x_{\stoabbr{max}}` для сокращённого
индекса. Команды работают внутри `$...$` и `$$...$$`. Вектор и матрица
выводятся прямым полужирным начертанием; единица, элемент и индекс — прямым.
Автор указывает роль символа сам: генератор не определяет её по буквам.
...
В реферате можно явно указать основные характеристики и область применения результата:
...
Команды размещаются под заголовком `РЕФЕРАТ`. Проверка исходника предупреждает об отсутствующих полях и отклоняет
пустые или повторные. Содержательную точность формулировок подтверждает автор отчёта.

Раздел `ОПРЕДЕЛЕНИЯ, ОБОЗНАЧЕНИЯ И СОКРАЩЕНИЯ` при необходимости размещается после содержания и до введения.
Его строки задаются в одном блоке, в алфавитном порядке, через `|`: термин, объяснение и необязательная единица.
Генератор выводит две колонки, добавляя тире перед объяснением.
...
Бета | коэффициент отклонения | рад
\end{sto_terms}
```

Внутритекстовое перечисление можно задать явно: `simple` для простых пунктов с запятой между ними и `complex`
для составных пунктов с точкой с запятой. Завершающий знак предложения пишется после команды.

```text
Признаки: \sto_inline_list{simple}{цвет|форма|размер}.
Параметры: \sto_inline_list{complex}{цвет, измеренный утром|форма, полученная расчётом}.
```
