...
Каждый запуск атомарно обновляет диагностический receipt (по умолчанию рядом с manifest, с суффиксом
`.diagnostics.json`) перед потенциально блокирующими COM-операциями. Если один этап не сообщает прогресс 120 секунд
или весь запуск длится больше 5 минут, watchdog останавливает только PowerShell и процесс Word этого запуска по проверенным
PID. Частичные DOCX/PDF/manifest удаляются; receipt сохраняет последний этап (`document.pdf-export`, `word.quit` и
т. п.), точную ошибку и результат адресной очистки. Успешная приемка остается успешной только после закрытия Word,
повторного открытия DOCX, стабильного числа страниц и записи всех трех артефактов.
